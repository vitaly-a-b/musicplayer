...
abstract class BaseModel extends BaseModelMethods
{
    ...
    public function connect()
    {
        ...
        if(!$this->db instanceof \mysqli){
            ...
            $this->timeout = round(microtime(true) * 1000);
...
//                $this->connect();
//
//                return $this->query($query, $crud, $return_id);
//
//            }

            if($this->bufferingErrors) return $this->db->errno;
    ...
    public function get($table, $set = []){
        ...
        $query = "SELECT $distinct $fields FROM `$table` $join $where $group $order $limit";
        ...
        if($res && !empty($set['single']) && count($res) === 1)
            $res = $res[key($res)];
    ...
    final public function showTables($clear = false){

        if($this->projectTables && !$clear)
    ...
    // фиксирование изменения закешированных таблиц,
    // $table - таблица данные которой изменились в БД. Вызывается в методах add, delete, edit
    protected function changeCacheData($table){
    ...
    protected function setCache($table){
        ...
        if ($this->checkCacheTable()){
            ...
            if($this->checkGeneralSettings()){
                ...
                if (empty($this->showColumns('general_settings')['cache_time']) && method_exists($this, 'createColumns')){
    ...
    // проверяем доступность таблицы необходимой для системы кеширования
    protected function checkCacheTable(){

        if(!in_array('cached_tables', $this->showTables(true))){
            ...
            $query = "CREATE TABLE cached_tables (
                ...
                date datetime null,
                cache_time  int null,
                constraint cached_tables_pk primary key (id)
                )";

            if($this->query($query, 'c')){

                $queryIndex = "CREATE UNIQUE INDEX cached_tables_unique_name_uindex 
...
// проверяем доступность таблицы необходимой для системы кеширования
    public function checkGeneralSettings(){
        ...
        if(!in_array('general_settings', $this->showTables(true))) {

            $query = "CREATE TABLE general_settings (
                id int auto_increment,
                cache_time  int null,
                constraint general_settings_pk primary key (id)
                )";
    ...
    public function getRealTables($tablesArr = []){
        ...
        foreach ($tablesArr as $table => $data){
            ...
            if(in_array($table, $this->showTables())){
                $result[$table] = $dateTime;
            }
