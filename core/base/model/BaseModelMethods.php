    ...
    protected function createWhere($set, $table = false, $instruction = 'WHERE'){
        ...
        if(!empty($set['where'])){
            ...
            foreach ($set['where'] as $key => $item) {
                ...
                preg_match('/^\s*([\(\)]*\{(.+?)\})/', $key, $matches0);
                preg_match('/^\s*(\W+)/', $key, $matches1);
                ...
                if(strtolower($operand) === 'between'){
                    ...
                    $item = !preg_match('/(rlike)/i', $operand) ? $this->clearValues($item) : "'" . trim($item) . "'";;
                ...
                $key = preg_match('/[\(\)]/', $key) ? $key : $table . '`' . $key . '`';
            ...
            foreach ($fields as $i => $item){
                ...
                foreach ($currentField as $row => $value){
                    ...
                    if(!$check_fields){
                        ...
                        if($duplicate) $insert_arr['duplicate'] .= '`' . $row . '`=VALUES(`' . $row . '`),';
