            ...
            <div class="wq-block__controls wq-controls">
                <div class="wq-controls__button wq-button wq-button_lavender wq-button_file _btn">
                    <?php if($this->showButtons()):?>
                        <input type="file" name="<?=$row?>" <?php if ($row === 'link') echo 'accept="audio/mpeg"'?> class="wq-block__input-file single_img">
                    <?php endif;?>
                    выбрать
                </div>
            ...
            if(!empty($this->data[$row])){
                ...
                if($info){
                    ...
                    if(preg_match('/image/i', $info)){
                        ...
                        $fileName = '<img src="' . PATH . UPLOAD_DIR . $this->data[$row] . '?v1.' . (str_replace(' ', '_', microtime())) . '" alt="service" ' . (preg_match('/\.svg$/', $this->data[$row]) ? 'style="min-width: 320px"' : '') . '>';
