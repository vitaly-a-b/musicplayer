            ...
            if(empty($arr['alias'])){
                ...
                if(!empty($rowAlias[$this->table]) && !empty($arr[$rowAlias[$this->table]])){
                    ...
                    foreach($arr as $key => $item){
                        if(mb_strpos($key, 'name') !== false && $item){
                            $link_str = $item;
                            break;
                        }
                    }
                ...
                if(empty($this->foreignData)){
                    ...
                    foreach ($this->foreignData as $row => $foreignArr){
                        ...
                        if(!empty($arr[$row]) && trim(strtolower((string)$arr[$row])) !== 'null' && !empty($foreignArr)){
                            ...
                            $fullName = $targetElement['recursive_name'] ?? ($targetElement['name'] ?? '');
        ...
        if(empty($this->userData['manual_menu_position'])){
            ...
            if(array_key_exists('menu_position', $_POST)){
                if(array_key_exists('parent_id', $_POST)){
                    $this->model->updateMenuPosition($this->table, 'menu_position', $where, $_POST['menu_position'], ['where' => 'parent_id'], $old_data);
                }else{
                    $this->model->updateMenuPosition($this->table, 'menu_position', $where, $_POST['menu_position'], [], $old_data);
                }
            }
